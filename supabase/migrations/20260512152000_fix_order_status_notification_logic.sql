-- Fix order status notifications to fire on every status change, not just the first paid transition
-- This ensures customers receive updates for 'shipped', 'delivered', etc., even if they already received 'processing'
...
BEGIN
  -- 1. Determine if this is a status change
  v_status_changed := (OLD.status IS DISTINCT FROM NEW.status);

  -- 2. Determine if this is the transition into a "Paid/Success" state
  -- We want to count it exactly once for statistics and commissions
  ...
  -- 3. Update customer statistics and commissions exactly once
  ...
  -- 4. Trigger WhatsApp notification on EVERY status change
  -- This ensures 'shipped' and 'delivered' updates are sent even if 'processing' was already sent
  IF v_status_changed THEN
    -- Fetch config with hardcoded fallbacks for reliability (matching existing pattern)
    ...
    EXCEPTION WHEN OTHERS THEN
      -- Allow transaction to continue even if notification fails
