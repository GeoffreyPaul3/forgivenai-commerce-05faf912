...
serve(async (req) => {
  ...
  try {
    ...
    if (action === 'sync') {
      ...
      console.log(`Extracted ${extractedProducts.length} unique products.`);

      // Step 3: Insert into Database with UPSERT (on name)
      ...
      return new Response(
        JSON.stringify({ success: true, imported: extractedProducts.length }),
